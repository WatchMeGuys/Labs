...
def createConfig():
    ...
    config['services'] = {'jwt_tool_version': jwttoolvers,
                          '# To disable the proxy option set this value to: False (no quotes)': None,
    ...
    try:
        if config['services']['proxy'] == "False":
            if config['argvals']['postData']:
                response = requests.post(url, data=config['argvals']['postData'], headers=headers, cookies=cookiedict,
                                         proxies=False, verify=False)
            else:
...
def injectheaderclaim(headerclaim, injectionvalue):
    ...
    newHeadB64 = base64.urlsafe_b64encode(json.dumps(newheadDict, separators=(",", ":")).encode()).decode(
        'UTF-8').strip("=")
...
def signingToken(newheadDict, newpaylDict):
    if config['argvals']['sigType'][0:2] == "hs":
        ...
        newSig, newContents = signTokenRSA(newheadDict, newpaylDict, config['crypto']['privkey'],
        ...
        newSig, newContents = signTokenEC(newheadDict, newpaylDict, config['crypto']['ecprivkey'],
                                          int(config['argvals']['sigType'][2:]))
        desc = "Tampered token - EC Signing:"
        jwtOut(newContents + "." + newSig, "Manual Tamper - EC Signing", desc)
    elif config['argvals']['sigType'][0:2] == "ps":
        newSig, newContents = signTokenPSS(newheadDict, newpaylDict, config['crypto']['privkey'],
                                           int(config['argvals']['sigType'][2:]))
    ...
    if testSig == sig:
        ...
        cprintc(
            "You can tamper/fuzz the token contents (-T/-I) and sign it using:\npython3 jwt_tool.py [options here] -S " + str(
                headDict["alg"]) + " -p \"" + key.decode('UTF-8') + "\"", "cyan")
        return cracked
    else:
        cracked = False
        if quiet == False:
...
def scanModePlaybook():
    ...
    jwtOut(newContents + "." + newSig, "Exploit: Spoof JWKS (-X s)",
           "Signed with JWKS at " + config['services']['jwksloc'])
    if origjku:
    ...
    if args.targeturl:
        if args.cookies or args.headers:
            if args.cookies and args.headers:
                if re.search('eyJ[A-Za-z0-9_\/+-]*\.eyJ[A-Za-z0-9_\/+-]*\.[A-Za-z0-9._\/+-]*',
                             args.cookies) and re.search(
                        'eyJ[A-Za-z0-9_\/+-]*\.eyJ[A-Za-z0-9_\/+-]*\.[A-Za-z0-9._\/+-]*', str(args.headers)):
            ...
            if args.headers:
                try:
                    ...
                    exit(1)
            try:
                findJWT = re.search('eyJ[A-Za-z0-9_\/+-]*\.eyJ[A-Za-z0-9_\/+-]*\.[A-Za-z0-9._\/+-]*',
                                    str(args.cookies) + " | " + str(args.headers))[0]
            except:
